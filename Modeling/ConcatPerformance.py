...
for folder in folders:
    ...
    if "_" not in folder:
        shutil.move(os.path.join(f'{master_folder}/{folder}'), f'{master_folder}/Originals')
    else:
        shutil.move(os.path.join(f'{master_folder}/{folder}'), f'{master_folder}/Transformed')
...
def concatenate(master_folder, folder):
...
for t_folder in transf_folders:
    for o_folder in org_folders:
        if str(o_folder + '_') in t_folder:
            o_rf, o_bag, o_xgb, o_logr, o_nn = concatenate(orig_folder, o_folder)
            t_rf, t_bag, t_xgb, t_logr, t_nn = concatenate(transf_folder, t_folder)
